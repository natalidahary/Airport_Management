...
int	saveBinaryFileCompressedCompany(const Airline* pComp, char* fileName)
{
	...
	BYTE data[2];
	int len = (BYTE)strlen(pComp->name);
	data[0] = (BYTE)pComp->flightCount >> 1;
	data[1] = (BYTE)pComp->flightCount << 7 | (BYTE)pComp->sortOpt << 4 | (BYTE)strlen(pComp->name);
	if (!fwrite(&data, sizeof(BYTE), 2, f))
	{
		fclose(f);
		return 0;
	}
	if (!fwrite(pComp->name, sizeof(char), len, f))
	{
		fclose(f);
		return 0;
	}
	...
	{
		if (!saveBinaryFileCompressedFlight(pComp->flightArr[i], f))
		{
			fclose(f);
			return 0;
		}
...
int readBinaryFileCompressedCompany(Airline* pComp, const char* fileName)
{
	...
	if (!fread(pComp->name, sizeof(char), len, f))
	{
		free(pComp->name);
		fclose(f);
		return 0;
	...
	for (i = 0; i < pComp->flightCount; i++)
	{
		...
		if (!readBinaryFileCompressedFlight(pComp->flightArr[i], f))
		{
