...
int main()
{
	...
	//saveBinaryFileCompressedCompany(&company, AIRLINE_COMPRESS_FILE_NAME);

	if (compress)
	{
		if (!saveBinaryFileCompressedCompany(&company, airlineFileName))
			printf("Error saving airline to a compress file");
	}
	else
	{
		if (!saveAirlineToFile(&company, airlineFileName))
			printf("Error saving airline to binary file");
	}
	freeManager(&manager);
	freeCompany(&company);
